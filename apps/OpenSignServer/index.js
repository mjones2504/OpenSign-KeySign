...
const allowedOrigins = ['https://keysign.usekeys.co'];
...
if (!process.env.TESTING) {
  ...
  httpServer.listen(port, '0.0.0.0', function () {
    ...
    const migrate = isWindows
      ? `set APPLICATION_ID=${serverAppId}&& set SERVER_URL=https://p02--keysign--46qt8mw4frvn.code.run/api/app&& set MASTER_KEY=${process.env.MASTER_KEY}&& npx parse-dbtool migrate`
      : `APPLICATION_ID=${serverAppId} SERVER_URL=https://p02--keysign--46qt8mw4frvn.code.run/api/app MASTER_KEY=${process.env.MASTER_KEY} npx parse-dbtool migrate`;
    setTimeout(() => {
